...
class Student:
    ...
    def __init__(self, name, id, grades = []):  # constructor
        ...
        self.id = str(id)
        self.grades = []
        self.addGrades(grades)

        print(f"Student Added: {self.name}  |  ID # {self.id} | {len(self.grades)} Grades recorded")
...
studentsDicts = [{'Name':'Betty Hemmings','ID':'1735','scores':95},
                {'Name':'James Hemmings','ID':'1765','scores':[50,7,89]},
                {'Name':'Sally Hemmings','ID':'1773','scores':[29,87,24,69]},
                {'Name':'Adalbert Frincke Hotopp', 'ID': '456', 'scores':[65,91,80]}
                ]
student_output = []

for sd in studentsDicts:
    s = Student(name=sd['Name'], id=sd['ID'], grades=sd['scores'])
    s.addGrades(random.randint(1, 100))
    s.addGrade([85])
    s.addGrades('thomas jefferson')
    print(s.average())
    s.showGrades()
    ...
    print('-----------------------------------------------------------------------------------')
    del(s)
    #create temp object and then append to student_output
